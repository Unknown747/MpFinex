...
func (c *Client) Connect() {
        ...
        // AUDIT: InsecureSkipVerify=true karena MT5 trade server menggunakan
        // self-signed certificate. Koneksi tetap terenkripsi TLS; hanya
        // verifikasi chain CA yang dilewati. Pertimbangkan pinning sertifikat
        // untuk lingkungan produksi.
        conn, err := tls.DialWithDialer(
                &net.Dialer{Timeout: 15 * time.Second},
                "tcp",
                c.Config.Host,
                &tls.Config{InsecureSkipVerify: true, ServerName: host}, //nolint:gosec
        )
        if err != nil {
                c.Status = StatusFailed
