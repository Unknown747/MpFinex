...
const (
	...
	kindSession    = "SESSION    "
	kindError      = "ERROR      "
	kindOrderFail  = "ORDER_FAIL "
	kindDrawdown   = "DRAWDOWN   "
	kindDailyPL    = "DAILY_PL   "
)
...
// OrderFailed logs ketika order ditolak beserta alasan lengkapnya.
// Contoh alasan: margin tidak cukup, volume terlalu kecil, jam news blackout.
func (l *Logger) OrderFailed(botID int, symbol, reason string) {
	l.write(kindOrderFail, fmt.Sprintf("bot=%d symbol=%s reason=%q", botID, symbol, reason))
}

// DrawdownSnapshot mencatat snapshot drawdown floating setiap 5 menit.
// Format: DRAWDOWN | current_equity=X peak_equity=Y drawdown_pct=Z%
func (l *Logger) DrawdownSnapshot(currentEquity, peakEquity, drawdownPct float64) {
	l.write(kindDrawdown, fmt.Sprintf(
		"current_equity=%.2f peak_equity=%.2f drawdown_pct=%.2f%%",
		currentEquity, peakEquity, drawdownPct,
	))
}

// DailyPL mencatat ringkasan P&L harian saat pergantian hari (tengah malam).
// Format: DAILY_PL | today_profit=X today_loss=Y win_rate=Z%
func (l *Logger) DailyPL(todayProfit, todayLoss, winRate float64) {
	l.write(kindDailyPL, fmt.Sprintf(
		"today_profit=%.2f today_loss=%.2f win_rate=%.1f%%",
		todayProfit, todayLoss, winRate,
	))
}
